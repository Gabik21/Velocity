...
public enum ProtocolUtils {
  ...
  /**
   * Reads an byte array for legacy version 1.7
   *
   * @param buf the buffer to read from
   * @return the read byte array
   */
  public static byte[] readArray17(ByteBuf buf) {
   ...
   */
  public static void writeArray17(byte[] b, ByteBuf buf, boolean allowExtended) {
    // (Integer.MAX_VALUE & 0x1FFF9A ) = 2097050 - Forge's current upper limit
    if (allowExtended) {
      Preconditions
          .checkArgument(b.length <= (Integer.MAX_VALUE & 0x1FFF9A),
              "Cannot send array longer than 2097050 (got %s bytes)", b.length);
    } else {
